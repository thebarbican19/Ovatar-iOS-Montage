...
-(void)viewWillLayoutSubviews {
    ...
    self.viewLabel.text = NSLocalizedString(@"Permissions_Gallery_Text", nil);
    ...
    self.viewAction = [[OActionButton alloc] initWithFrame:CGRectMake(self.view.bounds.size.width - 220.0, self.view.bounds.size.height - 102.0, 210.0, 90.0)];
    self.viewAction.backgroundColor = [UIColor clearColor];
    self.viewAction.clipsToBounds = false;
    self.viewAction.delegate = self;
    self.viewAction.title = NSLocalizedString(@"Permissions_Allow_Action", nil);
...
-(void)viewActionTapped:(OActionButton *)action {
    [self.delegate viewStorySetup:true];
