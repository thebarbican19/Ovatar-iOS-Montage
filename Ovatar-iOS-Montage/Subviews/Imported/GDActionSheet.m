...
#define MAIN_MODAL_HEADER 112.0
...
-(void)presentActionAlert {
    ...
    if (!self.textFont) self.textFont = [UIFont fontWithName:@"Avenir-Black" size:11.0];
    ...
    self.mainGradient.colors = @[(id)[UIColorFromRGB(0xF4F6F8) colorWithAlphaComponent:0.0].CGColor, (id)[UIColorFromRGB(0xF4F6F8) colorWithAlphaComponent:0.7].CGColor];
    ...
    if (self.buttons.count > 0) {
        for (int i = 0;i < self.buttons.count; i++) {
            ...
            actionButton.backgroundColor = self.warningAction?UIColorFromRGB(0xFF2323):UIColorFromRGB(0xFFDC00);
            ...
            CAGradientLayer *actionGradient = [CAGradientLayer layer];
            actionGradient.frame = actionButton.bounds;
            actionGradient.colors = @[(id)UIColorFromRGB(0x938DEF).CGColor, (id)UIColorFromRGB(0x7096F0).CGColor];
            actionGradient.startPoint = CGPointMake(0.0, 1.0);
            actionGradient.endPoint = CGPointMake(1.0, 0.0);
            [actionButton.layer addSublayer:actionGradient];
            
            UILabel *actionLabel = [[UILabel alloc] initWithFrame:CGRectMake(4.0, 1.0, actionButton.bounds.size.width - 8.0, actionButton.bounds.size.height - 2.0)];
            actionLabel.textColor = [UIColor whiteColor];
            actionLabel.attributedText = [self format:[[self.buttons objectAtIndex:i] objectForKey:@"title"]];
            actionLabel.textAlignment = NSTextAlignmentCenter;
            actionLabel.font = self.textFont;
    ...
    dismissButton.backgroundColor = self.warningAction?[UIColor whiteColor]:UIColorFromRGB(0x757585);
    ...
    dismissButton.layer.shadowOpacity = 0.0;
    [dismissButton setAttributedTitle:[self format:self.cancelText.uppercaseString] forState:UIControlStateNormal];
    [dismissButton setTitleColor:self.warningAction?UIColorFromRGB(0x140F26):UIColorFromRGB(0xFFFFFF) forState:UIControlStateNormal];
...
-(NSMutableAttributedString *)format:(NSString *)button {
    NSMutableAttributedString *attributed = [[NSMutableAttributedString alloc] initWithString:button.uppercaseString];
    [attributed addAttribute:NSKernAttributeName value:@1.2 range:NSMakeRange(0, button.length)];
    [attributed addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, button.length)];
